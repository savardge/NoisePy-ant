...
folder_name = 'TEMPEST'; % Folder where .mat files are
...
v_perturb = 30/100; % Velocity perturbation of the anomalies for checkerboard model 
nb_el = 5;  % number of elements defining the size of the checkerboard pattern. 
...
sigma = 3; % trade-off parameter between data fit and model complexity. Higher values fit data more.
...
set(im,'facealpha',0.6,'alphadata',mask') % apply mask
shading('flat');
plot(x_stat,y_stat,'vw','linewidth',0.5,'markersize',5, 'MarkerFaceColor','k')
cb = colorbar;
ylabel(cb, 'Velocity [km/s]')
...
% Save
fig_fname = [folder_name, '/checkerboard_T' num2str(T) '_mul' num2str(mul_lambda_thresh) '_' num2str(nb_el) 'x' num2str(nb_el) '.png'];
