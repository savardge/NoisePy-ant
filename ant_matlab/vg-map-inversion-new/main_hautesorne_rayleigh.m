function []=main_hautesorne_rayleigh(period) %period, sigma, LC)
%
%
%
tstart = tic;
...
LC_vec = [1];
sigma_vec = [0.02 0.015 0.01]

wave = 'rayleigh'

output_root = '/home/users/s/savardg/scratch/matlab-swant/data-hautesorne/run2_all_g500m/vg-maps-new'

save_output = true

%% Parameters
% STATIONS
station_file = '/home/users/s/savardg/hautesorne/noisepy/HS_ANT_stations_locations_from_log_median_wName.csv';
...
% PICKS
% V3
pick_file_csv = ['/home/users/s/savardg/scratch/hautesorne/dispersion-picks/picks_GaussFiltPeriod_1bitcoh_' wave '_lambda1.5_SNR10.0_margin0.5_multiple3_nodups_median.csv'];

max_std_percent = 20; %R:20
min_count = 3; %R: 3; L:2
...
fprintf('Number of picks/ray paths: %d\n', size(data,1))
...
% Background
background_file = '/home/users/s/savardg/scratch/matlab-swant/data-hautesorne/run0_best300_g500m/grid/hautesorne_background_terrain_faults.png';
...
% Plot wells
% [x_well, y_well] = ll2xy_sphericalproj(latwell,lonwell,grid_origin);
% hwell = scatter(x_well, y_well, 80, "red", 'filled', 'hexagram', 'MarkerEdgeColor', 'k', 'LineWidth',2);
 
set(gca,'linewidth',1.5,'fontsize',14) %,'layer','top')   
xlabel('Easting (km)', 'FontSize',14); 
ylabel('Northing (km)','FontSize',14);
title(sprintf('T = %.1f s, sigma = %.2e, LC = %.2e, dx=dy= %.1f km\n%d picks used',[period, sigma, LC, dx_grid]))
...
extent = [xmax, ymax];
fprintf('Grid origin (SW corner) lat,long: ( %.4f, %.4f )\nGrid extent: %.2f km by %.2f km\n', min_lat, min_lon, xmax, ymax)
...
if ~ismember('id',stations.Properties.VariableNames)
    stations.id = strcat(stations.network, '.', stations.station);
end
nsta = height(stations);
fprintf('Number of stations inside grid: %d (%d removed).\n', nsta, nsta0-nsta)
