% New script to extract the inversion results for all 2D grid cells and
% combine the output in one file for plotting. compatible when using the
% slurm-compatible version of the C script
% (C_launch_inversion_slurmarray.m)
% 
...
%% USER INPUTS
output_root = '../../data-aargau/run3_dcV2_mul2_g500m'
output_fold = [output_root, '/vs-model/run0_dv60_dz50m_N100_10L'];
% Output file to write with combined inversion output
output_file = [output_fold '/combined_result.mat'] 

% Get grid info
load([output_root '/grid/kernel.mat'], 'x_grid', 'y_grid', 'dx_grid', 'dy_grid')
ind_x_range = 1:length(x_grid); 
ind_y_range = 1:length(y_grid);

%% Get model params from output_ind_lin*.mat files (prior ranges, # layers, etc)

% load([output_fold '/params_inv.mat'], 'vg_pick_mat', 'T_pick_interp','V_range_mat', 'Z_range_mat'); % In old C script, prior ranges are extracted from params_inv.mat
flist = dir([output_fold '/output_ind_lin*.mat']);
load(fullfile(flist(1).folder,flist(1).name),'T_pick_interp', 'V_range_mat', 'Z_range_mat') % In new C script, the ranges are written in each output file.
nb_layer = size(Z_range_mat,1);
nb_profile = length(x_grid)*length(y_grid);
...
dz_grid = 10; % interpolation spacing along depth in m
...
%% Extract profiles one by one from output_ind_lin*.mat files

for ind_lin = 1:nb_profile

    if mod(ind_lin,100) == 0
        disp([num2str(ind_lin) '/' num2str(nb_profile)])
    end
    fname = [output_fold '/output_ind_lin_' num2str(ind_lin) '.mat'];
    if ~exist(fname,'file')
        continue
    end
    load(fname, 'misfit_merge', 'P_merge', 'disp_mat_merge');
        
    %% best fit = vg_lin_min / vs_lin_min
    ...
    %% combined solution = vg_lin_sol / vs_lin_sol
    
    % Option 1: Keep N best models
    N_keep = 100;
    [misfit_merge_sorted, ind_sorted] = sort(misfit_merge,'ascend');
    bool_keep = ind_sorted(1:N_keep);
    
    % Option 2: Keep models with misfit below threshold
