...
def plot_substack_cc_alltime(spair, ipath, sfiles,freqmin,freqmax,disp_lag=None,savefig=True,sdir='./', figname=None):
    '''
...
#         print(ii,np.max(np.abs(ndata[ii])))
    ...
    # save figure or show
    if savefig:
        outfname = sdir+'/moveout_allstack_'+str(stack_method)+'_'+str(ccomp)+'_bp'+str(freqmin)+'-'+str(freqmax)+'_'+str(dist_inc)+'kmbin.pdf'
        fig.savefig(outfname, format='pdf', dpi=400)
        plt.close()
    else:
        plt.show() # GS
...
def plot_all_moveout_1D_9comp(sfiles,sta,dtype,freqmin,freqmax,disp_lag=None,savefig=False,sdir=None,mdist=25,figsize=(14,11)):
    '''
    display the moveout waveforms of the cross-correlation functions stacked for all time chuncks.
