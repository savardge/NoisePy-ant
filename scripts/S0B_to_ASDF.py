...
DATADIR   = '/home/users/s/savardg/scratch/aargau/CLEAN_DATA_CH-AA'                         # dir where cleaned data in ASDF format are going to be outputted
locations = '/home/users/s/savardg/aargau_ant/station_locations_CH-AA.csv'       # station info including network,station,channel,latitude,longitude,elevation
...
respdir    = "/home/users/s/savardg/scratch/aargau/resp"                               # directory where resp files are located (required if rm_resp is neither 'no' nor 'inv')
freqmin   = 0.2                                                        # pre filtering frequency bandwidth
...
wiki_file = os.path.join(rootpath,'allfiles_time_CH-AA.csv')                  # file containing the path+name for all sac/mseed files and its start-end time
...
end_date   = ['2021_01_06_0_0_0']                                       # end date of local data
inc_hours  = 12                                                          # sac/mseed file length for a continous recording
...
MAX_MEM   = 12.0                                                         # maximum memory allowed per core in GB
