...
output_dir_root = os.path.join(rootpath,
                               f"dispersion_V2")  # dir where to output dispersive image and extracted dispersion
...
for stack_method in stack_methods:
    ...
    for comp in rtz_system:
        ...
        for lag in ["pos", "neg", "sym"]:
            amp, per, vel = dispersion.get_disp_image(CCFdata["ccf"][lag][comp], dist, dt, Tmin=Tmin, dT=dT, vmin=vmin,
