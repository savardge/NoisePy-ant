...
def generate_phase(stackEGF, fs, vmin, vmax, dvel, Tmin, Tmax, dT, dist, npts, Time, dt):
    ...
    PhaseVIm = time_shift(WinWaveClip, WaveClipPt, fs, Vpoint, Tmin, dT, dist, dt, NumCtrT, g_WinMinPtNum,
                          g_WinMaxPtNum, FilterKaiserPara=6, MaxFilterLengthLog=13)
...
def time_shift(WinWaveClip, WaveClipPt, fs, Vpoint, Tmin, dT, dist, dt, NumCtrT, g_WinMinPtNum, g_WinMaxPtNum,
               FilterKaiserPara=6, MaxFilterLengthLog=13):
    ...
    for i in range(NumCtrT):
        ...
        PhaseVIm.append(interp1d(TravPtV[::-1], (PhaseIm[i][int(g_WinMinPtNum):int(g_WinMaxPtNum)])[::-1], kind='cubic',
                                 bounds_error=False, fill_value=0)(Vpoint))
