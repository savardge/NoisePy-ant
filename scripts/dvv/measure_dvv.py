...
from noisepy import dvv_monitoring
...
para = {'twin': twin, 'freq': freq, 'dt': dt, 'ccomp': ccomp, 'onelag': onelag, 'norm_flag': norm_flag,
        'npts_all': npts_all, 'npts_win': npts_win}
...
with pyasdf.ASDFDataSet(sfile, mode='r') as ds:
    ...
    for ifreq in range(nfreq):
        ...
        ax0 = plt.subplot(311)
        # 2D waveform matrix
        ax0.matshow(tcur[:igood, disp_indx], cmap='seismic', extent=[tvec_disp[0], tvec_disp[-1], nwin, 0],
                    aspect='auto')
        ...
        ax0.fill(np.concatenate((tvec_all[nwin_indx], np.flip(tvec_all[nwin_indx], axis=0)), axis=0), \
                 np.concatenate((np.ones(len(nwin_indx)) * 0, np.ones(len(nwin_indx)) * nwin), axis=0), 'c', alpha=0.3,
                 linewidth=1)
        ...
        for ii in range(nwin):
            ...
            if do_stretch:
                dvv_stretch[ii, 0], dvv_stretch[ii, 1], cc, cdp = dvv_monitoring.stretching(pref, pcur, epsilon,
                                                                                            nbtrial, para)
                dvv_stretch[ii, 2], dvv_stretch[ii, 3], cc, cdp = dvv_monitoring.stretching(nref, ncur, epsilon,
                                                                                            nbtrial, para)
            if do_dtw:
                dvv_dtw[ii, 0], dvv_dtw[ii, 1], dist = dvv_monitoring.dtw_dvv(pref, pcur, para, mlag, b, direct)
                dvv_dtw[ii, 2], dvv_dtw[ii, 3], dist = dvv_monitoring.dtw_dvv(nref, ncur, para, mlag, b, direct)
            ...
            if do_mwcs:
                dvv_mwcs[ii, 0], dvv_mwcs[ii, 1] = dvv_monitoring.mwcs_dvv(pref, pcur, move_win_sec, step_sec, para)
                dvv_mwcs[ii, 2], dvv_mwcs[ii, 3] = dvv_monitoring.mwcs_dvv(nref, ncur, move_win_sec, step_sec, para)
            if do_mwcc:
                dvv_wcc[ii, 0], dvv_wcc[ii, 1] = dvv_monitoring.wcc_dvv(pref, pcur, move_win_sec, step_sec, para)
                dvv_wcc[ii, 2], dvv_wcc[ii, 3] = dvv_monitoring.wcc_dvv(pref, pcur, move_win_sec, step_sec, para)

            allfreq = False  # average dv/v over the frequency band for wts and wxs
            if do_wts:
                dvv_wts[ii, 0], dvv_wts[ii, 1] = dvv_monitoring.wts_dvv(pref, pcur, allfreq, para, epsilon, nbtrial,
                                                                            dj, s0, J, wvn)
                dvv_wts[ii, 2], dvv_wts[ii, 3] = dvv_monitoring.wts_dvv(nref, ncur, allfreq, para, epsilon, nbtrial,
                                                                            dj, s0, J, wvn)
            if do_wxs:
                dvv_wxs[ii, 0], dvv_wxs[ii, 1] = dvv_monitoring.wxs_dvv(pref, pcur, allfreq, para, dj, s0, J)
                dvv_wxs[ii, 2], dvv_wxs[ii, 3] = dvv_monitoring.wxs_dvv(nref, ncur, allfreq, para, dj, s0, J)
            ...
            if do_wts:
                dfreq,dv_wts1,unc1 = dvv_monitoring.wts_allfreq(ref[pwin_indx],cur[pwin_indx],allfreq,para,epsilon,nbtrial,dj,s0,J,wvn)
                dfreq,dv_wts2,unc2 = dvv_monitoring.wts_allfreq(ref[nwin_indx],cur[nwin_indx],allfreq,para,epsilon,nbtrial,dj,s0,J,wvn)
            if do_wxs:
                dfreq,dv_wxs1,unc1 = dvv_monitoring.wxs_allfreq(ref[pwin_indx],cur[pwin_indx],allfreq,para,dj,s0,J)
                dfreq,dv_wxs2,unc2 = dvv_monitoring.wxs_allfreq(ref[nwin_indx],cur[nwin_indx],allfreq,para,dj,s0,J)
            '''
