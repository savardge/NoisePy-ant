...
with pyasdf.ASDFDataSet(sfile,mode='r') as ds:
    ...
    for ifreq in range(nfreq):
        ...
        for ii in range(nwin):
            ...
            if do_stretch:
                dvv_stretch[ii,0],dvv_stretch[ii,1],cc,cdp = dvv_monitoring.stretching(pref,pcur,epsilon,nbtrial,para)
                dvv_stretch[ii,2],dvv_stretch[ii,3],cc,cdp = dvv_monitoring.stretching(nref,ncur,epsilon,nbtrial,para)
            if do_dtw:
            ...
            if do_wts:
                dvv_wts[ii,0],dvv_wts[ii,1] = dvv_monitoring.wts_allfreq(pref,pcur,allfreq,para,epsilon,nbtrial,dj,s0,J,wvn)
                dvv_wts[ii,2],dvv_wts[ii,3] = dvv_monitoring.wts_allfreq(nref,ncur,allfreq,para,epsilon,nbtrial,dj,s0,J,wvn)
            if do_wxs:
                dvv_wxs[ii,0],dvv_wxs[ii,1] = dvv_monitoring.wxs_allfreq(pref,pcur,allfreq,para,dj,s0,J)
                dvv_wxs[ii,2],dvv_wxs[ii,3] = dvv_monitoring.wxs_allfreq(nref,ncur,allfreq,para,dj,s0,J)
